...
namespace _77NeoWeb.Forms.Operaciones
{
    public partial class FrmOperacionLicencia : System.Web.UI.Page
    {
        ...
        protected void IdiomaControles()
        {
            ...
            using (SqlConnection sqlCon = new SqlConnection(Cnx.BaseDatosPrmtr()))
            {
                ...
                while (tbl.Read())  //Todos los objetos
                {
                    ...
                    GrdDatos.Columns[2].HeaderText = bO.Equals("GrdVlrH") ? bT : GrdDatos.Columns[2].HeaderText;
                }
                sqlCon.Close();
        ...
        protected void BindData(string VbConsultar, string Accion)
        {
            ...
            if (Accion.Equals("UPD"))
            {
                ...
                {
                    string VbTxtSql = "EXEC SP_TablasGeneral 20, '','','','','','','','','SELECT',0,0,0,0,0,@ICC,'01-01-1','02-01-1','03-01-1'";
                    sqlCon.Open();
        ...
        protected void GrdDatos_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName.Equals("AddNew"))
            {
                ...
                double VbVlrH = Convert.ToDouble((GrdDatos.FooterRow.FindControl("TxtVlrHPP") as TextBox).Text.Trim().Equals("") ? "0" : (GrdDatos.FooterRow.FindControl("TxtVlrHPP") as TextBox).Text.Trim());
                ...
                using (SqlConnection sqlCon = new SqlConnection(Cnx.GetConex()))
                {
                    ...
                    {
                        VBQuery = "EXEC SP_TablasGeneral 20,@Desc,@US, @Cd,'','','','','','INSERT', @Id,@Vlr,0,0,0,@ICC,'01-01-1','02-01-1','03-01-1'";
                        using (SqlCommand SC = new SqlCommand(VBQuery, sqlCon, Transac))
                        {
                            ...
                            SC.Parameters.AddWithValue("@Vlr", VbVlrH < 0 ? 0 : VbVlrH);
                            SC.Parameters.AddWithValue("@ICC", Session["!dC!@"]);
                            try
                            {
        ...
        protected void GrdDatos_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            double VbVlrH = Convert.ToDouble((GrdDatos.Rows[e.RowIndex].FindControl("TxtVlrH") as TextBox).Text.Trim().Equals("") ? "0" : (GrdDatos.Rows[e.RowIndex].FindControl("TxtVlrH") as TextBox).Text.Trim());
            if (VbDesc == String.Empty)
            {
                DataRow[] Result = Idioma.Select("Objeto= 'MstrMens06'");
                foreach (DataRow row in Result)
            ...
            using (SqlConnection sqlCon = new SqlConnection(Cnx.GetConex()))
            {
                ...
                using (SqlTransaction Transac = sqlCon.BeginTransaction())
                {
                    VBQuery = "EXEC SP_TablasGeneral 20,@Desc,@US, @Cd,'','','','','','UPDATE',@ID,@Vlr,0,0,0,@ICC,'01-01-1','02-01-1','03-01-1'";
                    using (SqlCommand SC = new SqlCommand(VBQuery, sqlCon, Transac))
                    {
                        SC.Parameters.AddWithValue("@Desc", VbDesc);
                        SC.Parameters.AddWithValue("@US", Session["C77U"].ToString());
