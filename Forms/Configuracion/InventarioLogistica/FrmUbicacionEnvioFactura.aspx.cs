...
{
    public partial class FrmUbicacionEnvioFactura : System.Web.UI.Page
    {
        ...
        protected void IdiomaControles()
        {
            ...
            using (SqlConnection sqlCon = new SqlConnection(Cnx.BaseDatosPrmtr()))
            {
                ...
                while (tbl.Read())  //Todos los objetos
                {
                    ...
                    GrdDatos.Columns[2].HeaderText = bO.Equals("GrdTip") ? bT : GrdDatos.Columns[2].HeaderText;
        ...
        protected void BindData(string VbConsultar, string Accion)
        {
            ...
            if (Accion.Equals("UPD"))
            {
                ...
                {
                    string VbTxtSql = "EXEC SP_TablasGeneral 6, '','','','','','','','EnvioFac','SELECT',0,0,0,0,0, @ICC,'01-01-1','02-01-1','03-01-1'	";
                    sqlCon.Open();
            ...
            if (DT.Rows.Count > 0)
            {
                ...
                DV.Sort = "CodIdTipoUbicaFac DESC";
                DT = DV.ToTable();
        ...
        protected void GrdDatos_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName.Equals("AddNew"))
            {
                ...
                VbTipU = (GrdDatos.FooterRow.FindControl("TxtipoUPP") as TextBox).Text.Trim().ToUpper();
                if (!VbTipU.Trim().Equals("I"))
                {
                    if (!VbTipU.Trim().Equals("N"))
                    {
                        DataRow[] Result = Idioma.Select("Objeto= 'Mens01UF'");
                        foreach (DataRow row in Result)
                        { ScriptManager.RegisterClientScriptBlock(this.Page, this.Page.GetType(), "alert", "alert('" + row["Texto"].ToString() + "');", true); }//El tipo solo permite la "N" nacional o "I" internacional.
                        return;
                    }
                }
                Cnx.SelecBD();
                using (SqlConnection sqlCon = new SqlConnection(Cnx.GetConex()))
                {
                    ...
                    {
                        VBQuery = "EXEC SP_TablasGeneral 6,@Desc,@US,'',@Tip,'','','','EnvioFac','INSERT',@Act,0,0,0,0, @ICC,'01-01-1','02-01-1','03-01-1'";
                        using (SqlCommand SC = new SqlCommand(VBQuery, sqlCon, Transac))
                        {
                            ...
                            SC.Parameters.AddWithValue("@Tip", VbTipU);
                            SC.Parameters.AddWithValue("@ICC", Session["!dC!@"]);
                            try
                            {
        ...
        protected void GrdDatos_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            VbTipU = (GrdDatos.Rows[e.RowIndex].FindControl("TxtipoU") as TextBox).Text.Trim().ToUpper();
            if (!VbTipU.Trim().Equals("I"))
            {
                if (!VbTipU.Trim().Equals("N"))
                {
            ...
            using (SqlConnection sqlCon = new SqlConnection(Cnx.GetConex()))
            {
                ...
                using (SqlTransaction Transac = sqlCon.BeginTransaction())
                {
                    VBQuery = "EXEC SP_TablasGeneral 6,@Desc,@US,@ID,@Tp,'','','','EnvioFac','UPDATE',@Act,0,0,0,0, @ICC,'01-01-1','02-01-1','03-01-1'";
                    using (SqlCommand SC = new SqlCommand(VBQuery, sqlCon, Transac))
                    {
                        SC.Parameters.AddWithValue("@Desc", VbDesc);
                        SC.Parameters.AddWithValue("@US", Session["C77U"].ToString());
                        ...
                        SC.Parameters.AddWithValue("@Tp", VbTipU);
                        SC.Parameters.AddWithValue("@ICC", Session["!dC!@"]);
                        try
                        {
                            var Mensj = SC.ExecuteScalar();
        ...
        protected void GrdDatos_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            using (SqlConnection sqlCon = new SqlConnection(Cnx.GetConex()))
            {
                ...
                using (SqlTransaction Transac = sqlCon.BeginTransaction())
                {
                    VBQuery = "EXEC SP_TablasGeneral 6,'',@US,@ID,'','','','','EnvioFac','DELETE',0,0,0,0,0, @ICC,'01-01-1','02-01-1','03-01-1'";
                    using (SqlCommand SC = new SqlCommand(VBQuery, sqlCon, Transac))
                    {
                        SC.Parameters.AddWithValue("@US", Session["C77U"].ToString());
                        SC.Parameters.AddWithValue("@ID", GrdDatos.DataKeys[e.RowIndex].Value.ToString());
