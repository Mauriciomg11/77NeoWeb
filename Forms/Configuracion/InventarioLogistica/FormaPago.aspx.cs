    ...
    public partial class FormaPago : System.Web.UI.Page
    {
        ...
        protected void IdiomaControles()
        {
            ...
            using (SqlConnection sqlCon = new SqlConnection(ConfigurationManager.ConnectionStrings["PConexDBPpal"].ConnectionString))
            {
                ...
                while (tbl.Read())  //Todos los objetos
                {
                    ...
                    GrdDatos.Columns[2].HeaderText = bO.Equals("GrdDia") ? bT : GrdDatos.Columns[2].HeaderText;
        ...
        protected void BindData(string VbConsultar, string Accion)
        {
            ...
            if (Accion.Equals("UPD"))
            {
                ...
                {
                    string VbTxtSql = "EXEC SP_TablasGeneral 6, '','','','','','','','FormaPago','SELECT',0,0,0,0,0, @ICC,'01-01-1','02-01-1','03-01-1'	";
                    sqlCon.Open();
        ...
        protected void GrdDatos_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName.Equals("AddNew"))
            {
                ...
                if ((GrdDatos.FooterRow.FindControl("TxtDiaPP") as TextBox).Text.Trim().Equals("")) { VbDia = Convert.ToDouble(0); }
                else { VbDia = Convert.ToDouble((GrdDatos.FooterRow.FindControl("TxtDiaPP") as TextBox).Text.Trim()); }
                Cnx.SelecBD();
                using (SqlConnection sqlCon = new SqlConnection(Cnx.GetConex()))
                {
                    sqlCon.Open();
                    ...
                    {
                        VBQuery = "EXEC SP_TablasGeneral 6,@Desc,@US,'','','','','','FormaPago','INSERT',@Act,@Dia,0,0,0, @ICC,'01-01-1','02-01-1','03-01-1'";
        ...
        protected void GrdDatos_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            if ((GrdDatos.Rows[e.RowIndex].FindControl("TxtDia") as TextBox).Text.Trim().Equals("")) { VbDia = Convert.ToDouble(0); }
            else { VbDia = Convert.ToDouble((GrdDatos.Rows[e.RowIndex].FindControl("TxtDia") as TextBox).Text.Trim()); }
            if (VbDesc == String.Empty)
            {
                DataRow[] Result = Idioma.Select("Objeto= 'MstrMens06'");
                foreach (DataRow row in Result)
            ...
            using (SqlConnection sqlCon = new SqlConnection(Cnx.GetConex()))
            {
                ...
                using (SqlTransaction Transac = sqlCon.BeginTransaction())
                {
                    VBQuery = "EXEC SP_TablasGeneral 6,@Desc,@US,@ID,'','','','','FormaPago','UPDATE',@Act,@Dia,0,0,0, @ICC,'01-01-1','02-01-1','03-01-1'";
                    using (SqlCommand SC = new SqlCommand(VBQuery, sqlCon, Transac))
                    {
                        SC.Parameters.AddWithValue("@Desc", VbDesc);
                        SC.Parameters.AddWithValue("@US", Session["C77U"].ToString());
        ...
        protected void GrdDatos_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            using (SqlConnection sqlCon = new SqlConnection(Cnx.GetConex()))
            {
                ...
                using (SqlTransaction Transac = sqlCon.BeginTransaction())
                {
                    VBQuery = "EXEC SP_TablasGeneral 6,'',@US,@ID,'','','','','FormaPago','DELETE',0,0,0,0,0, @ICC,'01-01-1','02-01-1','03-01-1'";
                    using (SqlCommand SC = new SqlCommand(VBQuery, sqlCon, Transac))
                    {
                        SC.Parameters.AddWithValue("@US", Session["C77U"].ToString());
                        SC.Parameters.AddWithValue("@ID", GrdDatos.DataKeys[e.RowIndex].Value.ToString());
