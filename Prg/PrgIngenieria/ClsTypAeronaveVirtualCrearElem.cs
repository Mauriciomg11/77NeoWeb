                ...
                using (SqlTransaction transaction = SCX.BeginTransaction())
                {
                    ...
                    using (SqlCommand SC = new SqlCommand(VBQuery, SCX, transaction))
                    {
                        ...
                        catch (Exception Ex)
                        {
                            ...
                            Cnx.UpdateErrorV2(VbUsu, VbPantalla, "ClsTypAeronaveVirtualCrearElem", Ex.StackTrace.Substring(Ex.StackTrace.Length > 300 ? Ex.StackTrace.Length - 300 : 0, 300), Ex.Message, VbcatVer, VbcatAct);
